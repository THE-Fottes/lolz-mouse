function createComplaint(username, link, userlink) {
	window.open(`https://zelenka.guru/forums/801/create-thread?${username}&${link}&${userlink}`, '_blank');
}

function setMouse() {
	for (var i = 0; i < $(".message").length; i++) {
		if ($('.message')[i].id.search("post") != -1 && typeof $(`.message > div.messageInfo > div.messageContent > div > div.publicControls > a:nth-child(5)`)[i] == "undefined") {
	...
	for (var i = 0; i < $(".comment").length; i++) {
		if ($('.comment')[i].id.search("post") != -1 && typeof $(`.comment > div.commentInfo > div.commentControls > div > a.item.control._ratButton`)[i] == "undefined") {
